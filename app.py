...
def postprocess(outputs, original_image, conf_thresh=0.1):
    ...
    for pred in predictions:
        ...
        if score > conf_thresh:
            ...
            if class_id >= len(CLASS_NAMES):
                print(f"⚠️ Skipping unknown class_id: {class_id}")
                continue

            print(f"✅ Detected: {CLASS_NAMES[class_id]} with score {score:.2f}")

            x1 = int((x_center - w / 2) * original_image.shape[1] / 640)
            y1 = int((y_center - h / 2) * original_image.shape[0] / 640)
            x2 = int((x_center + w / 2) * original_image.shape[1] / 640)
            y2 = int((y_center + h / 2) * original_image.shape[0] / 640)
